...
def get_record_data_by_opinion_id(_id, dict_citations, dict_counts):
    ...
    record['CL_opinion_date_created'] = '' if 'date_created' not in opinion else opinion['date_created']
    record['CL_opinion_date_modified'] = '' if 'date_modified' not in opinion else opinion['date_modified']
    record['CL_opinion_download_url'] = '' if 'download_url' not in opinion else opinion['download_url']
    record['CL_opinion_extracted_by_ocr'] = False if 'extracted_by_ocr' not in opinion else opinion['extracted_by_ocr']
    record['CL_opinion_local_path'] = None if 'local_path' not in opinion else opinion['local_path']
    record['CL_opinion_page_count'] = None if 'page_count' not in opinion else opinion['page_count']
    record['CL_opinion_per_curiam'] = False if 'per_curiam' not in opinion else opinion['per_curiam']
    record['CL_opinion_sha1'] = '' if 'sha1' not in opinion else opinion['sha1']
    record['CL_opinion_type'] = '' if 'type' not in opinion else opinion['type']
